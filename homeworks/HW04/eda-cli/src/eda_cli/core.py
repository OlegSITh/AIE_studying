...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        ...
        # Высокая кардинальность у категориальных (не числовых) колонок
        if not col.is_numeric and col.non_null > 0:
            unique_ratio = col.unique / col.non_null
            if unique_ratio > 0.5:   # >50% уникальных значений в категориальной колонке
